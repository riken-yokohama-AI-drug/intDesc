"""Utility for editing interaction parameter files."""
import argparse
import sys
...
def write_param(param_dict: dict, output: str):
    ...
    Args:
        param_dict (dict): Parameter dictionary (key=interaction name, value=list of parameters).
        output (str): Output file.
    """
    # List of interactions to be written
    # The output order follows this list.
...
def edit_param(parameter_file: str, buffer_dist: float) -> dict:
    """Read a parameter file and return a parameter dictionary with updated buffer distances.

    Args:
        parameter_file (str): Parameter file.
        buffer_dist (float): Buffer distance.

    Returns:
        dict: Parameter dictionary (key=interaction name, value=list of parameters).
    """

    # Interaction list that includes a buffer-distance parameter
    interaction_type_list = [
        "vdW",
        "(Met)_(X)",
        "(Ion)_(X)",
    ...
    try:
        ...
        with open(parameter_file, "r") as rf:
            ...
            for key in yml.keys():
                ...
                # Replace the buffer distance with the specified value
