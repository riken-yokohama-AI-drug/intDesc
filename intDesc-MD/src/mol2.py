...
class Mol2:
    ...
    def __init__(self, interaction_type):
        ...
        Args:
            interaction_type (str): Interaction calculation target type.
                                    Lig: ligand-protein interactions
                                    Mut: antibody-antigen interactions
                                    Met: medium-molecule interactions
        """
        # Member storing the @<TRIPOS>ATOM section
        ...
        # Member storing the @<TRIPOS>BOND section
        self.df_bond = pd.DataFrame(columns=["atom_id", "bond_atom_id", "bond_type"])

        # Member storing the @<TRIPOS>SUBSTRUCTURE section
    ...
    def read_mol2(self, mol2_file):
        ...
        Args:
            mol2_file (str): Path to the structural data file (.mol2) used for interaction descriptor calculation.
        """
        df_atom = None
        try:
        ...
        # Read the @<TRIPOS>BOND and @<TRIPOS>SUBSTRUCTURE sections
        ...
        # Assign backbone/sidechain classification to atoms
    ...
    def add_molcular_type(self, molcular_select_file):
        """Assign molecular types.

        Args:
            molcular_select_file (str): Molecule-selection definition file.
        """
        ...
        for mol_name in subst.keys():
            ...
            # NOTE: If specified ranges overlap, do not overwrite previously assigned values
            cond = cond & (self.df_atom["molcular_type"].isnull())

            if "protein" in mol_name:
                if self.interaction_type == "Med":
                    protein_no = ""
                    if "_" in mol_name:
                        protein_no = mol_name.split("_")[1]
                    self.df_atom.loc[cond, "molcular_type"] = f"Pro{protein_no}"
                    assigned_molcular_type[mol_name] = f"Pro{protein_no}"
                else:
                ...
                if self.interaction_type == "Med":
                    ligand_no = ""
                    if "_" in mol_name:
                        ligand_no = mol_name.split("_")[1]
                    self.df_atom.loc[cond, "molcular_type"] = f"L{ligand_no}"
                    assigned_molcular_type[mol_name] = f"L{ligand_no}"
                else:
                ...
                if self.interaction_type == "Med":
                    peptide_no = ""
                    if "_" in mol_name:
                        peptide_no = mol_name.split("_")[1]
                    self.df_atom.loc[cond, "molcular_type"] = f"Pep{peptide_no}"
                    assigned_molcular_type[mol_name] = f"Pep{peptide_no}"
                else:
            ...
            # Assign num_identity
            if "num_identity" in subst[mol_name].keys():
                num_identity = subst[mol_name]["num_identity"]
                if num_identity is None:
                    raise ValueError('Specify "num_identity".')
                elif isinstance(num_identity, list):
                    num_identity = ",".join(map(str, num_identity))
                else:
                    num_identity = str(num_identity)
                self.df_atom.loc[cond, "num_identity"] = num_identity
