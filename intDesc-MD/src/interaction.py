...
class Interaction:
    ...
    def __init__(
        ...
        Args:
            df_atom (DataFrame): DataFrame for the ATOM section of the structure used to compute interaction descriptors.
            df_bond (DataFrame): DataFrame for the BOND section of the structure used to compute interaction descriptors.
            interaction_parameter_file (str): Path to the interaction threshold settings file.
            vdw_difine_file (str): Path to the van der Waals radius definition file.
            exec_type (str): Execution mode.
        """
        columns = [
            "label1",
            "label2",
    ...
    def read_setting(self, interaction_parameter_file):
        """Load interaction threshold settings.

        Args:
            interaction_parameter_file (str): Path to the interaction parameter settings file.
        """
    ...
    def read_priority_file(self, priority_file):
        """Load interaction priority settings.

        Args:
            priority_file (str): Path to the interaction priority definition file.
        """
    ...
    def calculate(self, no_mediate, switch_ch_pi):
        """Run the interaction detection workflow.

        Args:
            no_mediate (bool): True to skip detection of solvent-mediated interactions.
            switch_ch_pi (bool): True to evaluate CH_PI, NH_PI, and OH_PI using the legacy definition.
        """
        if self.exec_type == "Lig":
            target_atoms = self.df_atom.loc[
                self.df_atom["molcular_type"] == "L",
                ["atom_id", "atom_type", "x", "y", "z", "resi", "molcular_type", "charge", "chain", "num_identity"],
            ]
            dist = 8.0
            ...
            target_atoms = self.df_atom.loc[
                self.df_atom["molcular_type"].str.contains("Pep|L|Pro", na=False),
                ["atom_id", "atom_type", "x", "y", "z", "resi", "molcular_type", "charge", "chain", "num_identity"],
            ]
            dist = 6.5
        ...
        for atom1 in target_atom1_atoms_list:
            ...
            atom1_chain = atom1[8]
            num_identity = [int(num.strip()) if num != "" else [] for num in atom1[9].split(",")]

            bond_atoms = self.df_bond.loc[
                self.df_bond["atom_id"] == atom1[0], "bond_atom_id"
            ].tolist()

            # NOTE: Conditions for atoms evaluated as interaction partners of atom_1
            # # Condition 1: located within the specified distance from atom_1
            # # Condition 2: exclude atoms covalently bonded to atom_1
            # # Condition 3: exclude atoms in the same residue as atom_1
            # When num_identity is specified, include the designated same-residue targets
            if atom1[5] in num_identity:
                if isinstance(atom1_chain, str):
                    atom2_cond = (
                        (
                            np.sqrt(
                                np.sum(
                        ...
                        & (~(self.df_atom["resi"] == atom1[5]) | ((self.df_atom["resi"] == atom1[5]) & (self.df_atom["chain"] == atom1_chain)))
                    )
                else:
                    atom2_cond = (
                        (
            ...
            if self.exec_type == "Med":
                ...
                # # Condition 4: target residues corresponding to atom_1 molcular_type
                atom2_mol_type_cond = None
                if "Pep" in atom1_molcular_type:
                    atom2_mol_type_cond = self.df_atom["molcular_type"].str.contains("Pep|S|Mem", na=False)

                elif "L" in atom1_molcular_type:

                    atom2_mol_type_cond = self.df_atom["molcular_type"].str.contains("Pro|S", na=False)

                elif "Pro" in atom1_molcular_type:
                    atom2_mol_type_cond = self.df_atom["molcular_type"].str.contains("S", na=False)

                atom2_cond = atom2_cond & atom2_mol_type_cond

            elif self.exec_type == "Lig":
                # # For ligand-protein interactions
                # # Condition 4: target matching residues (protein and solvent) for atom_1 molcular_type
                ...
                # # Condition 4: target entries with molcular_type assigned.
        ...
        # Detection of solvent-mediated interactions
        if no_mediate:
            return

        if self.exec_type == "Lig":
            # Detect interactions for solvent atoms involved in [Lig-S] interactions
        ...
        if self.exec_type == "Mut":
            # Detect interactions for solvent atoms involved in [Mut-S] interactions
            target_atoms_list = self.interaction_table.loc[
            ...
            # Detect interactions for solvent atoms involved in [Pep-S*, L-S*, Pro-S*] interactions
            target_atoms_list = self.interaction_table.loc[
        ...
        for atom1 in target_atoms:
            ...
            atom1_molcular_type = atom1[6]
            atom1_chain = atom1[8]
            num_identity = [int(num.strip()) if num != "" else num for num in atom1[9].split(",")]

            bond_atoms = self.df_bond.loc[
                self.df_bond["atom_id"] == atom1_id, "bond_atom_id"
            ].tolist()

            # NOTE: Conditions for atoms evaluated as interaction partners of atom_1 (solvent)
            # # Condition 1: located within the specified distance from atom_1
            # # Condition 2: exclude atoms covalently bonded to atom_1
            # # Condition 3: exclude atoms in the same residue as atom_1
            # When num_identity is specified, include the designated same-residue targets
            ...
            if self.exec_type == "Lig":
                ...
                atom2_cond = atom2_cond & (self.df_atom["molcular_type"].str.contains("Pep|Pro", na=False))
    ...
    def calc_interaction(
        ...
        """Run interaction evaluation for a pair of atoms.

        Args:
            atom1_id (int): atom_id of atom1.
            atom1_atom_type (string): Atom type of atom1.
            atom1_coord (float): Coordinates of atom1 (x, y, z).
            atom1_resi (int): Residue number of atom1.
            atom1_molcular_type (string): Molecular type of atom1.
            atom1_charge (float): Charge of atom1.
            atom2_id (int): atom_id of atom2.
            atom2_atom_type (string): Atom type of atom2.
            ...
            atom2_charge (float): Charge of atom2.
            switch_ch_pi (bool): True to evaluate CH_PI, NH_PI, and OH_PI using the legacy definition.
        """
    ...
    def update_interaction_table(
        ...
        Args:
            ...
            atom1_id (int): Serial atom number for atom1.
            atom1_molcular_type (str): molcular_type of atom1.
            atom2_id (int): Serial atom number for atom2.
            atom2_molcular_type (str): molcular_type of atom2.
            item_1 (float, optional): Interaction metric value. Defaults to np.nan.
            item_2 (float, optional): Interaction metric value. Defaults to np.nan.
            item_3 (float, optional): Interaction metric value. Defaults to np.nan.
            item_4 (float, optional): Interaction metric value. Defaults to np.nan.
            item_5 (float, optional): Interaction metric value. Defaults to np.nan.
            item_6 (float, optional): Interaction metric value. Defaults to np.nan.
            item_7 (float, optional): Interaction metric value. Defaults to np.nan.
            item_8 (float, optional): Interaction metric value. Defaults to np.nan.
            item_9 (float, optional): Interaction metric value. Defaults to np.nan.
            item_10 (float, optional): Interaction metric value. Defaults to np.nan.

        """
        # NOTE: Do not add non-mediated water/solvent interactions if already registered
        ...
        if any(
            [
                ...
                and any([
                    atom2_molcular_type in ["Mut", "Ab", "Ag"],
                    "Pro" in atom2_molcular_type,
                    "Pep" in atom2_molcular_type,
                ]),
                "S" in atom2_molcular_type
                and any([
                    atom1_molcular_type in ["Mut", "Ab", "Ag"],
                    "Pro" in atom1_molcular_type,
                    "Pep" in atom1_molcular_type,
                ]),
            ]
        ):
            # atom3: solvent atom
            # atom4: atom interacting with the solvent atom
            ...
            # Get the residue number of the solvent atom
            atom3_resi = int(self.df_atom.loc[self.df_atom["atom_id"] == atom3_id, "resi"].iloc[0])
            target_atom_list = self.df_atom.loc[
                self.df_atom["resi"] == atom3_resi, "atom_id"
            ].tolist()
            # Target DataFrame condition 1: interacts with the same water/solvent molecule and atom4 is not the same atom as atom1.
            condition = (
            ...
            if "Pep" in atom4_molcular_type:
                # Target DataFrame condition 1-2: update rows for "Pep-S" interactions.
                condition = condition & (
                    self.interaction_table["pair_type"].str.contains(f"Pep[0-9]*-{atom3_molcular_type}", na=False)
                )
                # condition = condition & (
                #     self.interaction_table["pair_type"] == f"{atom4_molcular_type}-{atom3_molcular_type}"
                # )
            elif "Pro" in atom4_molcular_type:
                if self.exec_type == "Med":
                    # Target DataFrame condition 1-2: update rows for "L-S*" and "Pro-S*" interactions.
                    condition = condition & (
                        (self.interaction_table["pair_type"].str.contains(f"L[0-9]*-{atom3_molcular_type}", na=False)) |
                        (self.interaction_table["pair_type"].str.contains(f"Pro[0-9]*-{atom3_molcular_type}", na=False))
                    )
                elif self.exec_type == "Lig":
                    # Target DataFrame condition 1-2: update rows for "L-S*" interactions.
                    condition = condition & (
                        self.interaction_table["pair_type"] == f"L-{atom3_molcular_type}"
                    )

            elif atom4_molcular_type in ["Mut", "Ab", "Ag"]:
                # Target DataFrame condition 1-2: update rows for "Mut-S*" interactions.
                condition = condition & (
            ...
            if self.exec_type == "Lig":
                # NOTE: For "S-Pro", update only as solvent-mediated interactions and do not register separately
                if atom4_molcular_type == "Pro":
                    return
            elif self.exec_type == "Mut":
                # NOTE: For "S-Ab" and "S-Ag", update only as solvent-mediated interactions and do not register separately
                if atom4_molcular_type in ["Ab", "Ag"]:
                    return

        # NOTE: Normalize pair_type values to the patterns below
        # # L*-Pro*, L*-S*, Mut-Ab, Mut-Ag, Mut-Mut, Mut-S*, Pep*-Pep*, Pep*-S*, Pep*-Mem, Pro*-S*
        if any(
            [
                "Pep" in atom1_molcular_type and "Pep" in atom2_molcular_type,
            ...
            [
                "L" in atom2_molcular_type,
                "Mut" in atom2_molcular_type,
                "Pep" in atom2_molcular_type,
                "S" in atom1_molcular_type and "Pro" in atom2_molcular_type,
            ]
        ):
            tmp_id = atom2_id
    ...
    def drop_13_14(self):
        ...
        # Annotate atom1 bond information with 1-4 related atom IDs
        df_bond = pd.merge(
            df_bond,
            self.df_bond[["atom_id", "bond_atom_id"]],
            how="left",
        ...
        for atom1 in atom_1_list:
            # Register interactions that are not in a 1-3 or 1-4 relationship with atom1
        ...
        # NOTE: Dipole-dipole interactions are defined by two atom-pair records; remove non-duplicated Dipo labels.
    ...
    def drop_duplicate(self):
        ...
        # Assign score values to the interaction table
        ...
        # Set the keep flag for non-duplicated atom pairs and Dipo interaction atom pairs.
        ...
        # For duplicated atom pairs, set the keep flag on the entry with the highest score.
        dup_pairs = (
            init_table.loc[
        ...
        # Remove duplicated interactions for atom3-atom4 pairs.
        dup_pairs = (
            init_table.loc[
                (
        ...
        for pair in dup_pairs:
            ...
            atom3_id = pair[2]
            atom4_id = pair[3]
            # NOTE: Get the maximum score value within duplicated combinations
            max_score = init_table.loc[
        ...
        # Re-register rows whose keep flag is True
    ...
    def drop_mediate_interaction(self, allow_mediate_position):
        """Remove solvent-mediated interaction records.

        For solvent-mediated interactions, remove records whose
        atom_2--atom_3 positional relationship exceeds the allowed range.

        Args:
            allow_mediate_position (int): Allowed positional relationship index between solvent atoms.
        """
        if self.interaction_table.empty:
            return
        ...
        # Get solvent-atom (atom2) information for solvent-mediated interactions
        atom_2_list = list(
            set(
        ...
        # Extract bond information for atom2
        df_bond = self.df_bond.loc[
        ...
        if allow_mediate_position > 2:
            # NOTE: Extend bond connectivity up to the allowed positional relationship
    ...
    def calc_hydrogen_bond1(
        ...
        # Return if atom1/atom2 do not satisfy the AtomType conditions
        if (
        ...
        # Must be bonded to an AtomType "H" atom (hydrogen atom)
        ...
        # Use a loop because atom1 may have multiple bonded atoms
        for atom1_bond in atom1_bonds:
            atom1_bond_atom_type = atom1_bond[0]
            if atom1_bond_atom_type[0] != "H":
                continue

            # Get coordinates of the bonded atom for atom1
            atom1_bond_coord = atom1_bond[1:]

            # Get the angle formed by atom2, atom1, and the atom bonded to atom1 (angle near atom1)
    ...
    def calc_hydrogen_bond1_oh(
        ...
        """Hydrogen bond type 1 (OH acceptor).
    ...
    def calc_hydrogen_bond2_oh(
        ...
        """Hydrogen bond type 2 (OH acceptor).
    ...
    def calc_ch_o(
        ...
        if len(atom2_bonds) == 1:
            ...
            # Generate combinations by Cartesian product of atoms bonded to atom2
            # # Combination rule 1: (atom_x, atom_h) = (heavy atom, hydrogen atom)
            # # Combination rule 2: (atom_x, atom_h) = (hydrogen atom, hydrogen atom) when no heavy atom exists in the bonded set
            for atom2_x, atom2_h in itertools.product(atom2_bonds, atom2_bonds):
    ...
    def calc_sh_n_sh_o(
        ...
        if atom2_atom_type == "N.ar":
            # For AtomType "N.ar", it must not be bonded to an "H" atom (hydrogen atom)
            if "H" in atom2_bonds[:, 0]:
                return

        # Evaluate all heavy atoms covalently bonded to atom1
        x_flag = False
        for atom1_bond_x in atom1_bonds_x:
            ...
            # Get the distance between atom1 BondX and atom2
        ...
        # Evaluate all hydrogen atoms covalently bonded to atom1
        h_flag = False
        ...
        # Evaluate all atoms covalently bonded to atom2
        atom2_flag = False
        if len(atom2_bonds) == 1:
    ...
    def calc_electrostatic_oh(
        ...
        """Electrostatic interaction (OH acceptor).
    ...
    def calc_van_der_waals(
        ...
        # Get hydrogen atoms covalently bonded to atom1
        atom1_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom1_id, "bond_atom_id"].tolist()
        atom1_targets = (
            self.df_atom.loc[
                (self.df_atom["atom_id"].isin(atom1_bonds)) & (self.df_atom["atom_type"] == "H"),
        ...
        # Get hydrogen atoms covalently bonded to atom2
        atom2_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom2_id, "bond_atom_id"].tolist()
        atom2_targets = (
            self.df_atom.loc[
    ...
    def calc_pi_pi_stacking(
        ...
        # Extract aromatic ring atoms belonging to atom1
        atom1_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom1_id, "bond_atom_id"].tolist()
        ...
        # Get atoms forming the aromatic rings containing Atom1 and Atom2
        atom1_cycles = get_cycle_atom(aroma_atoms1, atom1_id)
        atom2_cycles = get_cycle_atom(aroma_atoms2, atom2_id)

        # Process each atom_id list forming atom1 aromatic rings
        for atom1_cycle in atom1_cycles:
            ...
            # Intersection of a perpendicular from Atom2 to the aromatic plane containing Atom1
            ...
            # Normal vector of the aromatic plane containing Atom1
            nv1 = my_math.normal_vector_three_points(atom1_ar1, atom1_coord, atom1_ar2)

            # Process each atom_id list forming atom2 aromatic rings
            for atom2_cycle in atom2_cycles:
                ...
                # Normal vector of the aromatic plane containing Atom2
                nv2 = my_math.normal_vector_three_points(atom2_ar1, atom2_coord, atom2_ar2)

                # Angle between aromatic-plane normal vectors
    ...
    def calc_dipo_dipo(
        ...
        # If no bonded atoms are present
        if len(atom1_bonds) == 0:
            return None
        ...
        # Evaluate all atoms covalently bonded to atom1
        for atom1_bond in atom1_bonds:
            ...
            # Vector defined by atom1 and atom3
            if atom1_charge > 0:
            ...
            for atom2_bond in atom2_bonds:
                ...
                # Distance2 threshold check
                if distance2 > buffer_param + float(vdw_radius):
                    continue

                # Vector defined by atom2 and atom4
                if atom2_charge > 0:
                ...
                if all([angle1 >= angle1_param, angle2 <= angle2_param, angle3 <= angle3_param]):
                    ...
                    # NOTE: Dipole-dipole interactions are bond-based, so interaction records are defined for each atom pair.
                    self.update_interaction_table(
    ...
    def calc_omulpol(
        ...
        # Check bond conditions for Atom1
        atom1_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom1_id, "bond_atom_id"].tolist()
        atom1_bonds = self.df_atom.loc[
            self.df_atom["atom_id"].isin(atom1_bonds) & ~(self.df_atom["atom_type"] == "H"),
            ["charge", "x", "y", "z"],
        ...
        if len(atom2_bonds) == 1:
            ...
            # Generate combinations by Cartesian product of atoms bonded to atom2
            # # atom4: heavy atom with charge sign opposite to atom2
            # # atom5: heavy atom.
    ...
    def calc_xh_pi(
        ...
        """CH_PI, OH_PI, NH_PI, and SH_PI interactions.
        ...
        # atom1 bond conditions
        # The number of covalently adjacent atoms must be at least 2.
        atom1_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom1_id, "bond_atom_id"].tolist()
        atom1_bonds = (
            self.df_atom.loc[
                self.df_atom["atom_id"].isin(atom1_bonds),
        ...
        # Check distance conditions between atom2 and atoms bonded to atom1
        is_true = False
        distance2 = np.nan
        distance3 = np.nan
        angle2 = np.nan
        atom1_h_coord = None
        # Generate combinations by Cartesian product of atoms bonded to atom1
        # Combinations for CH_PI, NH_PI, and OH_PI
        # # Combination rule 1: (atom_x, atom_h) = (heavy atom, hydrogen atom)
        # # Combination rule 2: (atom_x, atom_h) = (hydrogen atom, hydrogen atom) when no heavy atom exists in the bonded set
        # Combination for SH_PI
        # # Combination rule 1: (atom_x, atom_h) = (heavy atom, hydrogen atom)
        ...
        for cycle_atom in cycle_atom_id:
            ...
            # Center of gravity of the aromatic ring
            ...
            # Point Nrm obtained by dropping a perpendicular from atom1 to the aromatic plane
            ...
            # Distance between point Nrm and the center of gravity
    ...
    def calc_ch_oh_nh_pi_two(
        ...
        """CH_PI, OH_PI, and NH_PI interactions (legacy definition).
        ...
        # Threshold check: distance between two points
        vdw_radius = Decimal(str(self.vdw_difine[atom1_symbol])) + Decimal(
            str(self.vdw_difine[atom2_symbol])
        )
        distance1 = my_math.distance_two_points(atom1_coord, atom2_coord)
        ...
        # Aromatic ring atoms adjacent to atom1
        ...
        for cycle_atom in cycle_atom_ids:
            # Center-of-gravity coordinates of the aromatic ring
            cycle_coords = [ar[1:] for ar in aroma_atoms if ar[0] in cycle_atom]
            c1 = my_math.center_of_gravity(cycle_coords)

            # Maximum distance from the aromatic-ring center of gravity to each ring atom plus the threshold distance
            r = 0
            ...
            # Get the intersection of a perpendicular dropped from atom2 onto the aromatic plane containing atom1
            ...
            # Intersection between atom2 and the normal vector perpendicular to the aromatic plane through atom1
    ...
    def calc_halogen_one(
        ...
        # atom1 bond conditions
        # The number of covalently adjacent atoms must be 1.
        atom1_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom1_id, "bond_atom_id"].tolist()
        if len(atom1_bonds) != 1:
            return
        atom1_bonds = (
        ...
        # atom2 bond conditions
        # The number of covalently adjacent atoms must be 1 or 2.
        atom2_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom2_id, "bond_atom_id"].tolist()
        if any(
            [
        ...
        if len(atom2_bonds) == 1 and has_heaby_atom2_bond:
            # Case where atom2 has one covalently bonded atom
            ...
            # Case where atom2 has two covalently bonded atoms
            # Combination rule 1: (atom_x, atom_h) = (heavy atom, heavy atom)
            # Combination rule 2: (atom_x, atom_h) = (heavy atom, hydrogen atom)
            # Combination rule 3: (atom_x, atom_h) = (hydrogen atom, hydrogen atom) when no heavy atom exists in the bonded set
    ...
    def calc_xh_f(
        ...
        """Detect CH_F, NH_F, OH_F, and SH_F interactions.
        ...
        # atom2 bond conditions
        # Must be covalently bonded to one heavy atom.
        atom2_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom2_id, "bond_atom_id"].tolist()
        ...
        # Generate combinations by Cartesian product of atoms bonded to atom1
        # Combination rule 1: (atom_x, atom_h) = (heavy atom, hydrogen atom)
        # Combination rule 2: (atom_x, atom_h) = (hydrogen atom, hydrogen atom) when atom1 is a water molecule
    ...
    def calc_xh_halogen(
        ...
        # atom1 bond conditions
        # The number of covalently adjacent atoms must be 2 or more.
        atom1_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom1_id, "bond_atom_id"].tolist()
        if len(atom1_bonds) < 2:
            return
        ...
        # Calculate criteria for two atoms covalently bonded to atom1
        distance2 = np.nan
        distance3 = np.nan
        angle1 = np.nan
        angle3 = np.nan
        ...
        for atom1_x, atom1_h in itertools.product(atom1_bonds, atom1_bonds):
            ...
            # If multiple candidates for atom_x exist, choose the one with the smallest distance2
            # If a shorter distance2 is obtained later, do not keep the current candidate even if it already meets the criteria.
    ...
    def calc_halogen_pi(
        ...
        for cycle_atom in cycle_atom_id:
            ...
            # Center of gravity of the aromatic ring
            cn = my_math.center_of_gravity(cycle_aroma_coords)
            # Distance between the center of gravity and each atom
    ...
    def calc_oh_s_sh_s(
        ...
        # atom2 bond conditions
        # The number of covalently adjacent atoms must be 1 or more.
        atom2_bonds = self.df_bond.loc[self.df_bond["atom_id"] == atom2_id, "bond_atom_id"].tolist()
        if len(atom2_bonds) == 0:
            return
    ...
    def calc_s_o(
        ...
        # Check distance conditions between atoms bonded to atom1 and atom2
        distance4 = np.nan
        distance5 = np.nan
        if len(atom2_bond_atoms) == 1:
            ...
            for atom2_bond_pair in itertools.combinations(atom2_bond_atoms, 2):
                ...
                atom2_y = atom2_bond_pair[1][1:]
                # Prefer a heavy atom for atom2_x
                if atom2_bond_pair[0][0] == "H" and atom2_bond_pair[1][0] != "H":
    ...
    def calc_s_f(
        ...
        # Distance1 and Distance4 checks
        if distance1 >= distance4:
            return None
        ...
        for atom1_bond in atom1_bonds_x:
            ...
            distance2 = my_math.distance_two_points(atom1_bond_coord, atom2_coord)
            # Distance1 and Distance2 checks
            if distance1 >= distance2:
                return None
            ...
            for atom1_bond_w in atom1_bonds_w:
                ...
                # Distance1 and Distance3 checks
    ...
    def calc_s_pi_1(
        ...
        for atom1_bond_pair in itertools.combinations(atom1_bonds, 2):
            ...
            # Prefer a heavy atom for atom1_x
            ...
            # Normal vector defined by atoms covalently bonded to atom1
        ...
        for cycle_atom in cycle_atom_id:
            ...
            # Center of gravity of the aromatic ring
            cn = my_math.center_of_gravity(cycle_aroma_coords)
            # Distance between atom2 and the center of gravity
            distance4 = my_math.distance_two_points(cn, atom2_coord)
            ...
            # Normal vector defined by aromatic atoms covalently bonded to atom2
    ...
    def write_one_hot_list(self, output, group_difine_file="group.yaml"):
        ...
        # Extract only "L-S-Pro" rows and remove duplicates
        ...
        # Get the list of non-solvent-mediated interactions
        lig_lists = (
            self.interaction_table.loc[
                self.interaction_table["label2"].isna(),
    ...
    def write_one_hot_list_medium(self, output, group_difine_file="group.yaml"):
        """Write a one-hot list file for the Medium mode.
        ...
        peptides = [mol_type for mol_type in molcular_types if "Pep" in mol_type]
        ligands = [mol_type for mol_type in molcular_types if "L" in mol_type]
        proteins = [mol_type for mol_type in molcular_types if "Pro" in mol_type]
        solvents = [mol_type for mol_type in molcular_types if "S" in mol_type]

        # Create one-hot-vector labels for each molecular-type pair
        label_columns = []

        # P[N]P[N], P[N]Mem, P[N]S[N]
        for peptide_1, peptide_2 in itertools.product(peptides, peptides):
            peptide_1 = peptide_1.replace("Pep", "P")
            peptide_2 = peptide_2.replace("Pep", "P")
            label_columns.extend([f"{peptide_1}{peptide_2}_{label}" for label in labels])
        for peptide in peptides:
            peptide = peptide.replace("Pep", "P")
            label_columns.extend([f"{peptide}Mem_{label}" for label in labels])
            for solvent in solvents:
                label_columns.extend([f"{peptide}{solvent}_{label}" for label in labels])

        # L[N]Pro[N], L[N]S[N]
        for ligand, protein in itertools.product(ligands, proteins):
            label_columns.extend([f"{ligand}{protein}_{label}" for label in labels])
        for ligand, solvent in itertools.product(ligands, solvents):
            label_columns.extend([f"{ligand}{solvent}_{label}" for label in labels])

        # Pro[N]S[N]
        for protein, solvent in itertools.product(proteins, solvents):
            label_columns.extend([f"{protein}{solvent}_{label}" for label in labels])
        ...
        # Extract "P-S-P", "L-S-Pro", and "Pro-S-Pro" rows and remove duplicates
        df_sol_interaction = self.interaction_table.loc[~(self.interaction_table["label2"].isna())]
        sol_inter_lists = (
            df_sol_interaction.loc[
                ~(
        ...
        for row in no_sol_inter_lists + sol_inter_lists:
            label = "Dipo" if "Dipo" in row[0] else row[0]
            ...
            if len(pair_type) == 3:
                if "Pro" in pair_type[2]:
                    # For "L-S-Pro" -> Pro[N]S[N]
                    if "L" in pair_type[0]:
                        mol_type = "protein"
                        partner_mol_type = "solvent"
                        one_hot_label = f"{pair_type[2]}{pair_type[1]}_{one_hot_label}"

                    # For "Pro-S-Pro" -> Pro[N2]S[N]
                    elif "Pro" in pair_type[0]:
                        mol_type = "protein"
                        partner_mol_type = "solvent"
                        one_hot_label = f"{pair_type[2]}{pair_type[1]}_{one_hot_label}"

                # For "P-S-P" -> P[N2]S[N]
                elif "Pep" in pair_type[2]:
                    mol_type = "peptide"
                    partner_mol_type = "solvent"
                    peptide_2 = pair_type[2].replace("Pep", "P")
                    one_hot_label = f"{peptide_2}{pair_type[1]}_{one_hot_label}"

            # Non-solvent-mediated interactions: Peptide
            elif "Pep" in pair_type[0]:
                # For "P-P" -> P[N]P[N]
                if "Pep" in pair_type[1]:
                    mol_type = "peptide"
                    partner_mol_type = "peptide"
                    peptide_1 = pair_type[0].replace("Pep", "P")
                    peptide_2 = pair_type[1].replace("Pep", "P")
                    one_hot_label = f"{peptide_1}{peptide_2}_{one_hot_label}"

                # For "P-S" -> P[N]S[N]
                elif "S" in pair_type[1]:
                    mol_type = "peptide"
                    partner_mol_type = "solvent"
                    peptide_1 = pair_type[0].replace("Pep", "P")
                    one_hot_label = f"{peptide_1}{pair_type[1]}_{one_hot_label}"

                # For "P-Mem" -> P[N]Mem
                else:
                    mol_type = "peptide"
                    partner_mol_type = "membrane"
                    peptide_1 = pair_type[0].replace("Pep", "P")
                    one_hot_label = f"{peptide_1}Mem_{one_hot_label}"

            # Non-solvent-mediated interactions: Ligand
            elif "L" in pair_type[0]:
                # For "L-Pro" -> L[N]Pro[N]
                if "Pro" in pair_type[1]:
                    mol_type = "ligand"
                    partner_mol_type = "protein"
                    one_hot_label = f"{pair_type[0]}{pair_type[1]}_{one_hot_label}"
                # For "L-S" -> L[N]S[N]
                else:
                    mol_type = "ligand"
                    partner_mol_type = "solvent"
            ...
            # Non-solvent-mediated interactions: Protein
            # For "Pro-S" -> Pro[N]S[N]
            else:
                mol_type = "protein"
                partner_mol_type = "solvent"
        ...
        df_one_hot = pd.DataFrame(one_hot_list)
        df_one_hot = df_one_hot.loc[:, ~(df_one_hot.sum(axis=0) == 0)]
        def delete_duplicate_data(row):
            # Pair information for the interaction
            pair1 = (row["residue"], row["residue_number"], row["atom_name"], row["atom_number"])
            pair2 = (row["partner_residue"], row["partner_residue_number"], row["partner_atom_name"], row["partner_atom_number"])
            main_info = (row["dist"], row["interaction_label"], row["molcular_type"])
            sorted_pair = tuple(sorted([pair1, pair2]))

            return main_info + sorted_pair

        # Add the key column to the DataFrame
        df_one_hot["duplicate_key"] = df_one_hot.apply(delete_duplicate_data, axis=1)

        # Remove duplicates (keep only one record)
        df_one_hot = df_one_hot.drop_duplicates(subset="duplicate_key").drop(columns="duplicate_key")
    ...
    def write_interaction_sum_list(self, output, group_difine_file="group.yaml"):
        ...
        # Extract only "L-S-Pro", remove duplicates, and count by interaction group
        df_sol_interaction = self.interaction_table.loc[~(self.interaction_table["label2"].isna())]
        sol_pro_lists = (
            df_sol_interaction.loc[
                ~(
        ...
        # Get non-solvent-mediated interactions and count them by interaction group
        label1_table = (
            self.interaction_table.loc[
                self.interaction_table["label2"].isna(),
                ["pair_type", "label1"],
    ...
    def write_interaction(self, mol2_name, output):
        """Write interaction detection results (Raw list file).
    ...
    def write_total_interaction(self, output, group_difine_file="group.yaml", is_output=True):
        """Write interaction count summary results (Interaction Count list file).

        Args:
            output (str): Output file prefix.
            group_difine_file (str): Interaction group definition file.
            is_output (bool): Switch controlling file output.
        """
        ...
        if self.exec_type == "Lig":
            ...
            for peptide_1, peptide_2 in itertools.product(peptides, peptides):
                ...
                peptide_2 = peptide_2.replace("Pep", "P")
                for label in labels:
                    total_list[f"{peptide_1}#{label}#{peptide_2}"] = 0

            for peptide in peptides:
                peptide = peptide.replace("Pep", "P")
                for label in labels:
                    total_list[f"{peptide}#{label}#Mem"] = 0

            for ligand, protein in itertools.product(ligands, proteins):
                for label in labels:
                    total_list[f"{ligand}#{label}#{protein}"] = 0

            for solvent in solvents:
                for peptide in peptides:
                    peptide = peptide.replace("Pep", "P")
                    for label in labels:
                        total_list[f"{peptide}#{label}#{solvent}"] = 0

                for ligand in ligands:
                    for label in labels:
                        total_list[f"{ligand}#{label}#{solvent}"] = 0

                for protein in proteins:
                    for label in labels:
                        total_list[f"{protein}#{label}#{solvent}"] = 0

                for peptide_1, peptide_2 in itertools.product(peptides, peptides):
                    peptide_1 = peptide_1.replace("Pep", "P")
                    peptide_2 = peptide_2.replace("Pep", "P")
                    for label_1, label_2 in itertools.product(labels, labels):
                        total_list[f"{peptide_1}#{label_1}#{solvent}#{label_2}#{peptide_2}"] = 0

                for ligand, protein in itertools.product(ligands, proteins):
                    for label_1, label_2 in itertools.product(labels, labels):
                        total_list[f"{ligand}#{label_1}#{solvent}#{label_2}#{protein}"] = 0

                for protein_1, protein_2 in itertools.product(proteins, proteins):
                    for label_1, label_2 in itertools.product(labels, labels):
                        total_list[f"{protein_1}#{label_1}#{solvent}#{label_2}#{protein_2}"] = 0

        # Aggregate detected interactions
        for row in table:
            label1 = row[0]
            if "Dipo" in label1:
                label1 = "Dipo"
        ...
        # NOTE: Dipole-dipole interactions are bond-based, so the total count is half the number of interaction records.
        for key, val in total_list.items():
            if "Dipo" in key:
                total_list[key] = int(val / 2)

        # Sort each item by label and write the output
    ...
    def write_pml(self, output, suffix, model_prefix, water_def_file):
        ...
        Args:
            output (str): Output file prefix.
            suffix (str): Suffix for group names and distance objects.
                          NOTE: Used to avoid collisions when visualizing multiple structures simultaneously.
            model_prefix (str): Name of the structure to visualize.
                                NOTE: Used when creating distance objects.
            water_def_file (str): Definition file for water molecule names.
        """
        ...
        def convert_pair_label(pair_type):
            ...
            labels = []
            if self.exec_type == "Med":
                is_in_mol_types1 = any(value in mol_types[0] for value in convert_dict[self.exec_type].values())
                if is_in_mol_types1:
                    atom1_mol_type = mol_types[0]
                else:
                    atom1_N_match = re.search(r'\d+$', mol_types[0])
                    atom1_mol_type_N = atom1_N_match.group() if atom1_N_match else ""
                    atom1_mol_type = mol_types[0][0]+atom1_mol_type_N
            else:
            ...
            if mol_types[1][0] == "S":
                ...
                if self.exec_type == "Med":
                    is_in_mol_types2 = any(value in mol_types[1] for value in convert_dict[self.exec_type].values())
                    if is_in_mol_types2:
                        atom2_mol_type = mol_types[1]
                    else:
                        atom2_N_match = re.search(r'\d+$', mol_types[1])
                        atom2_mol_type_N = atom2_N_match.group() if atom2_N_match else ""
                        atom2_mol_type = mol_types[1][0]+atom2_mol_type_N
                else:
            ...
            if len(mol_types) == 3:
                solvent = mol_types[1]
                other = mol_types[2] if self.exec_type == "Med" else convert_dict[self.exec_type].get(mol_types[2], mol_types[2][0])
                if mol_types[0][0] == mol_types[2][0]:
                    # NOTE: For M-S-M and P-S-P, normalize to MS and PS.
                    if self.exec_type == "Med":
                        labels.append("".join([mol_types[2].replace("Pep", "P"), solvent]))
                    else:
                        labels.append("".join([other, solvent]))
                elif "Pro" in other:
                    # NOTE: For medium mode with L-S-Pro, normalize to ProS.
                    labels.append("".join([other, solvent]))
                else:
        ...
        # Get atom_ids to be displayed as sticks
        target_type = None
        if self.exec_type == "Lig":
            target_type = "L"
        elif self.exec_type == "Mut":
            ...
            target_type = "Pep"
        stick_atom = (
            self.df_atom[self.df_atom["molcular_type"].str.contains(target_type, na=False)]["atom_id"]
            .to_numpy()
            .tolist()
        )
        ...
        with open(f"{output}.pml", "w", encoding="utf8") as file:
            ...
            for row in table:
                ...
                for label, atom1_id, atom2_id, pair_label in interaction_sets:
                    ...
                    # For dipole-dipole interactions, display only interactions between atoms with opposite charge signs
                    if "Dipo" in label:
